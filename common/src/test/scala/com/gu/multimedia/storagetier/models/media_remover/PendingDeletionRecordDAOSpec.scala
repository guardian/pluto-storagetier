...
class PendingDeletionRecordDAOSpec extends Specification with BeforeAll with AfterEach {
  ...
  "PendingDeletionRecordDAO.writeRecord" should {
    "insert a new record, and update it" in {
      ...
      val rec = PendingDeletionRecord(None, "some/file/path", Some("nearline-test-id"), Some("vsid"), MediaTiers.NEARLINE, 1)
    ...
    "fail if we try to update a record that does not exist" in {
      val rec = PendingDeletionRecord(Some(123), "some/file/path", Some("nearline-test-nonexistent-id"), Some("vsid"), MediaTiers.NEARLINE, 1)
  ...
  "NearlineRecordDAO.deleteRecord" should {
    "delete an existing record" in {
      val rec = PendingDeletionRecord(None, "some/file/path", Some("nearline-test-id-to-delete"), Some("vsid"), MediaTiers.NEARLINE, 1)
