    ...
    implicit val decodeAssetSweeperNewFile:Decoder[AssetSweeperNewFile] = new Decoder[AssetSweeperNewFile] {
      final def apply(c: HCursor): Result[AssetSweeperNewFile] = {
        for {
          ...
          ignore,
          mimeType,
          ZonedDateTime.ofInstant(Instant.ofEpochMilli((mTime*1000.0).toLong), ZoneId.systemDefault()),
          ZonedDateTime.ofInstant(Instant.ofEpochMilli((cTime*1000.0).toLong), ZoneId.systemDefault()),
          ZonedDateTime.ofInstant(Instant.ofEpochMilli((aTime*1000.0).toLong), ZoneId.systemDefault()),
          owner,
          group,
          filename,
          parentDir
