...
class FileCopierSpec extends Specification with Mockito {
  "FileCopier.maybeGetIndex" should {
    "return the index portion of a filename" in {
      ...
      val toTest = new FileCopier(mockChecksumHelper) {
  ...
  "FileCopier.copyFileToMatrixStore" should {
    "perform an upload without an existing object id and return new object id with checksum" in {
      ...
      val toTest = new FileCopier(mockChecksumHelper) {
        ...
        override protected def verifyChecksumMatchUsingHelper(filePath: Path, potentialFiles: Seq[MxsObject], maybeLocalChecksum: Option[String]): Future[Option[String]] = mockVerifyChecksum(filePath, potentialFiles, maybeLocalChecksum)
      }
...
//      val mockChecksumHelper = mock[ChecksumHelper]
//      val toTest = new FileCopier(mockChecksumHelper) {
...
//
//        def callVerify(filePath:Path, potentialFiles:Seq[MxsObject]) = verifyChecksumMatchUsingHelper(filePath, potentialFiles)
//      }
//
